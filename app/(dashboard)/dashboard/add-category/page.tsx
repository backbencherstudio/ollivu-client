...
export default function AddCategory() {
  ...
  const [searchSelectedCategory, setSearchSelectedCategory] = useState<string | null>(null);
  ...
  const handleAddCategory = async () => {
    ...
    if (categories.some(cat => cat.category_name.toLowerCase() === newCategory.toLowerCase())) {
  ...
  const handleAddSubCategory = async () => {
    ...
    try {
      ...
      // Debug log
      console.log('Sending request to:', `/categories/sub-category/${selectedCategoryId}`);
              ...
              <div className="flex-1 max-w-md space-y-4">
                ...
                {selectedCategoryId && (
                  <p className="text-sm text-gray-500">
                    Adding to: {categories.find(cat => cat._id === selectedCategoryId)?.category_name}
                  </p>
                )}
              </div>
              <Button
                        ...
                        {category.subCategories?.length > 0 ? (
                          <div className="grid grid-cols-2 gap-3">
                            {category.subCategories?.map((sub: any) => (
                              <div key={sub._id} className="p-3 bg-white rounded-lg shadow-sm flex items-center gap-3">
                                <img
                                  src={sub.categoryImage} // Changed back to categoryImage
                                  alt={sub.subCategory}

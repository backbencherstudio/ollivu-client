...
export const MessageContent = ({
  ...
  // ✅ Step 4: Filter messages by serial numbers
  const filterMessagesForChat = (messages, currentUser, currentChat) => {
    if (!currentChat || !currentUser) return [];

    return messages.filter((msg) => {
      const isMatchSenderSerial =
        msg.senderSerialNumber === currentChat?.senderSerialNumber;
      const isMatchReceiverSerial =
        msg.reciverSerialNumber === currentChat?.reciverSerialNumber;

      return isMatchSenderSerial && isMatchReceiverSerial;
    });
  };

  const filteredMessages = filterMessagesForChat(messages, currentUser, currentChat);
          ...
          >
            <div className="max-w-[85%] group">
              <div className="relative">
                ...
                {/* Services */}
                <div className="mt-1 text-[11px] flex items-center gap-3 text-gray-500">
                  <small>{msg?.senderService || "No service selected"}</small>
                  <ArrowRightLeft className="w-4 h-4" />
                  <small>{msg?.reciverService || "No service selected"}</small>
                </div>
